...
export default class Kick extends IzumiCommand {
	async run(msg: Message, args: Args) {
		...
		const row = new MessageActionRow().addComponents(new MessageButton().setCustomID('kickApprove').setLabel('Approve').setStyle('SUCCESS'), new MessageButton().setCustomID('kickDeny').setLabel('Deny').setStyle('DANGER'));
		...
		const filter = (interaction: MessageComponentInteraction) => ['kickApprove', 'kickDeny'].includes(interaction.customID) && interaction.user.id === msg.author.id;
		...
		collector.on('collect', async (interaction) => {
			switch (interaction.customID) {
				case 'kickApprove':
					await member.value.send(msg.embed.success.setDescription(`You have been kicked from \`${msg.guild?.name}\` (\`${msg.guild?.id}\`) for **${reason}**`)).catch((e) => console.log(e));
					let success = msg.embed.success.setDescription(`\`${member.value.user.tag}\` (\`${member.value.id}\`) has been kicked for **${reason}**`);
